/**
 * Serial implementation of the Sieve of Eratosthenes (only numbers ending in 1,3,7,9) for finding all the primes up to a given number.
 * 
...
int main(int argc, char const *argv[]) {
    ...
    if (args.max > 1) {
        ...
        // Output the results.
        printf("Serial-1379: Max = %.1le, %f seconds\n", (double) args.max, elapsed);
        if (args.shouldPrint)
        ...
        if (size > 1) {
            ...
            if (size > 2) {
                ...
                for (unsigned long i = 3; i < size; ++i) {
                    if (primes[i]) {
                        printf("%lu\n", (unsigned long) (i * 2.5 - offsets[offIndex]));
                    }
