...
int main(int argc, char const *argv[]) {
    ...
    if (args.max > 1) {
        ...
        // Output the results.
        printf("R-OMP: %d threads, Max = %lu, serial limit = %d, %f seconds\n", args.numThreads, args.max, serial_limit, elapsed);
        if (args.shouldPrint)
...
myArgs getArgs(int argc, char const *argv[]) {
    ...
    serial_limit = strtol(argv[3], NULL, 10);
...
void usage(const char *prog_name) {
    fprintf(stderr, "\nUsage: %s <thread count> <max> <serial limit> <1 to print primes>\n", prog_name);
...
void ompSieve(char primes[], int numThreads, unsigned long max) {
    ...
    if (max > 2) {
        ...
        #pragma omp parallel private(i) num_threads(numThreads) if(max > serial_limit)
        {
            for (i = 3; i < limit; i += 2) {
